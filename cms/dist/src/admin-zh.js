...
exports.applyAdminChinese = applyAdminChinese;
 ...
 */
function contentManagerStore(strapi) {
...
function buildFieldMetadata(field, attr, labels) {
...
function buildDefaultConfiguration(attributes, labels) {
    ...
    const mainField = fields.find((f) => fieldType(attributes[f]) === "string") ?? fields[0] ?? "documentId";
    return {
        settings: {
            bulkable: true,
            filterable: true,
 ...
 */
async function applyLabels(strapi, storeKey, attributes, labels) {
 ...
 */
async function applyAdminChinese(strapi) {
    ...
    try {
        ...
        strapi.log.warn(`[汉化] 统一管理员界面语言失败：${error.message}`);
    }
    // 2) 注入内容类型字段中文标签
    ...
    for (const [uid, labels] of Object.entries(CONTENT_TYPE_LABELS)) {
        ...
        try {
            await applyLabels(strapi, `${CONTENT_TYPE_KEY}::${uid}`, contentType.attributes, labels);
        }
        catch (error) {
            strapi.log.warn(`[汉化] 内容类型 ${uid} 标签注入失败：${error.message}`);
        }
    }
    ...
    for (const [uid, labels] of Object.entries(COMPONENT_LABELS)) {
        ...
        try {
            await applyLabels(strapi, `${COMPONENT_KEY}::${uid}`, component.attributes, labels);
        }
        catch (error) {
            strapi.log.warn(`[汉化] 组件 ${uid} 标签注入失败：${error.message}`);
        }
    }
    strapi.log.info("[汉化] 管理面板中文字段标签已应用");
}
//# sourceMappingURL=admin-zh.js.map