...
export function SiteFooter({ setting }: { setting?: SiteSetting | null }) {
  if (!setting) {
    return (
      <footer className="print-hidden w-full bg-gradient-to-r from-thu-purple-dark via-[#450550] to-thu-purple-dark py-8 text-center text-sm text-white/70 border-t-2 border-thu-gold/60">
        <p className="font-serif-title text-white/90 text-base">高校官网</p>
        <p className="mt-2 text-xs text-white/50">暂无站点信息</p>
      </footer>
    );
  }
  ...
  return (
    <footer className="print-hidden w-full text-sm border-t-2 border-thu-gold/80">
      {/* 主区域：深紫渐变底色 */}
      <div className="bg-gradient-to-b from-thu-purple via-[#560962] to-thu-purple-dark text-white/85 py-8 md:py-12">
        <div className="mx-auto max-w-6xl px-4 space-y-6">
          ...
          {/* 联系方式 */}
          <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 text-xs sm:text-sm text-white/75">
            {setting.address && (
      ...
      {(setting.icpRecordNo || setting.policeRecordNo) && (
        <div className="bg-thu-purple-dark text-white/50 text-xs py-3 border-t border-white/5">
